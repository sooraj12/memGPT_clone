...
class Message(Record):
    ...
    def __init__(
        ...
        if self.embedding is not None:
            assert (
                self.embedding_dim
            ), "Must specify embedding_dim if providing an embedding"
            assert (
                self.embedding_model
            ), "Must specify embedding_model if providing an embedding"
            assert (
    ...
    def dict_to_message(
        ...
        if openai_message_dict["role"] == "function":
            ...
            if openai_message_dict["role"] == "tool":
                ...
                if "tool_call_id" in openai_message_dict:
                    assert (
                        openai_message_dict["tool_call_id"] is None
                    ), openai_message_dict
    ...
    def to_anthropic_dict(self, inner_thoughts_xml_tag="thinking") -> dict:
        ...
        if self.role == "system":
            raise ValueError("Anthropic 'system' role not supported")
                ...
                content.append(
                    {
                        "type": "text",
                        "text": add_xml_tag(
                            string=self.text, xml_tag=inner_thoughts_xml_tag
                        ),
                    }
                )
            if self.tool_calls is not None:
    ...
    def to_google_ai_dict(self, put_inner_thoughts_in_kwargs: bool = True) -> dict:
        ...
        if self.role != "tool" and self.name is not None:
            raise UserWarning(
                f"Using Google AI with non-null 'name' field ({self.name}) not yet supported."
            )
