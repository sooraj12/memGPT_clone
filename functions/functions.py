...
from constants import MEMGPT_DIR, CLI_WARNING_PREFIX
...
def load_all_function_sets(merge: bool = True, ignore_duplicates: bool = True) -> dict:
    ...
    function_sets_dir = os.path.join(
        scripts_dir, "function_sets"
    )  # Path to the function_sets directory
    # List all .py files in the directory (excluding __init__.py)
    example_module_files = [
        f
        for f in os.listdir(function_sets_dir)
    ...
    user_module_files = [
        f
        for f in os.listdir(USER_FUNCTIONS_DIR)
        if f.endswith(".py") and f != "__init__.py"
    ]
            ...
            if dir_path == USER_FUNCTIONS_DIR:
                ...
                try:
                    ...
                    missing_package = str(e).split("'")[
                        1
                    ]  # Extract the name of the missing package
                    print(
                        f"{CLI_WARNING_PREFIX}skipped loading python file '{module_full_path}'!"
                    )
                    print(
                    ...
                    print(
                        f"{CLI_WARNING_PREFIX}skipped loading python file '{file}' due to a syntax error: {e}"
                    )
                    continue
                except Exception as e:
                    # Handle other general exceptions
                    print(
                        f"{CLI_WARNING_PREFIX}skipped loading python file '{file}': {e}"
                    )
                    continue
            else:
                ...
                try:
                    ...
                    # Handle other general exceptions
                    print(
                        f"{CLI_WARNING_PREFIX}skipped loading python module '{full_module_name}': {e}"
                    )
                    continue
