...
from local_llm.llm_chat_completion_wrappers.wrapper_base import LLMChatCompletionWrapper
...
class ConfigurableJSONWrapper(LLMChatCompletionWrapper):
    ...
    def _compile_system_message(
        self, system_message, functions, function_documentation=None
    ) -> str:
    ...
    def _compile_function_call(self, function_call, inner_thoughts=None):
        ...
        return json.dumps(
            airo_func_call, indent=self.json_indent, ensure_ascii=JSON_ENSURE_ASCII
        )
    ...
    def _compile_function_response(self, message) -> str:
        ...
        try:
            ...
            function_return_dict = json.loads(
                message["content"], strict=JSON_LOADS_STRICT
            )
    ...
    def chat_completion_to_prompt(
        self, messages, functions, first_message=False, function_documentation=None
    ):
        ...
        for message in messages:
            if message["role"] == "system":
                msg = self._compile_system_message(
                    message["content"], functions, function_documentation
                )
                ...
                if no_user_prompt_start:
                    ...
                    formatted_messages += (
                        self.user_prompt_start + msg + self.user_prompt_end
                    )
                ...
                formatted_messages += (
                    self.tool_prompt_start + msg + self.tool_prompt_end
                )
        ...
        if self.strip_prompt:
            if first_message:
                prompt = (
                    formatted_messages
                    + self.post_prompt
                    + self.assistant_prefix_extra_first_message
                )
            else:
                prompt = (
                    formatted_messages + self.post_prompt + self.assistant_prefix_extra
                )
            return prompt.strip()
        else:
            if first_message:
    ...
    def _clean_function_args(self, function_name, function_args):
        ...
        cleaned_function_args = (
            function_args.copy() if function_args is not None else {}
        )
    ...
    def output_to_chat_completion_response(self, raw_llm_output, first_message=False):
        assistant_prefix = (
            self.assistant_prefix_extra_first_message
            if first_message
            else self.assistant_prefix_extra
        )
        if (
            assistant_prefix
            and raw_llm_output[: len(assistant_prefix)] != assistant_prefix
        ):
        ...
        message = {
            ...
            "function_call": {
                ...
                "arguments": json.dumps(
                    function_parameters, ensure_ascii=JSON_ENSURE_ASCII
                ),
            },
        }
        return message
    ...
    def load_from_yaml(file_path: str):
        ...
        wrapper.assistant_prefix_extra_first_message = data.get(
            "assistant_prefix_extra_first_message", ""
        )
        ...
        wrapper.include_sys_prompt_in_first_user_message = data.get(
