...
def get_db_model(
    ...
    if table_type == TableType.ARCHIVAL_MEMORY or table_type == TableType.PASSAGES:
        ...
        class MessageModel(Base):
            ...
            model = Column(
                String
            )  # optional: can be null if LLM backend doesn't require specifying
...
class SQLStorageConnector(StorageConnector):
    ...
    def get_filters(self, filters: Optional[Dict] = {}):
        ...
        all_filters = [
            getattr(self.db_model, key) == value
            for key, value in filter_conditions.items()
        ]
        return all_filters

    def get_all_paginated(
        self, filters: Optional[Dict] = {}, page_size: Optional[int] = 1000, offset=0
    ) -> Iterator[List[RecordType]]:
        ...
        with self.session_maker() as session:
            ...
            if reverse:
                query = query.order_by(
                    desc(getattr(self.db_model, order_by)), asc(self.db_model.id)
                )
            else:
                query = query.order_by(
                    asc(getattr(self.db_model, order_by)), asc(self.db_model.id)
                )
            ...
            if after:
                ...
                query = query.filter(
                    or_(
                        sort_exp,
                        and_(
                            getattr(self.db_model, order_by) == after_value,
                            self.db_model.id > after,
                        ),
                    )  # tiebreaker case
                )
            if before:
                ...
                query = query.filter(
                    or_(
                        sort_exp,
                        and_(
                            getattr(self.db_model, order_by) == before_value,
                            self.db_model.id < before,
                        ),
                    )
                )
    ...
    def get_all(self, filters: Optional[Dict] = {}, limit=None) -> List[RecordType]:
        ...
        with self.session_maker() as session:
            if limit:
                db_records = (
                    session.query(self.db_model).filter(*filters).limit(limit).all()
                )
            else:
    ...
    def query(
        ...
        raise NotImplementedError(
            "Vector query not implemented for SQLStorageConnector"
        )
    ...
    def list_data_sources(self):
        assert (
            self.table_type == TableType.ARCHIVAL_MEMORY
        ), "list_data_sources only implemented for ARCHIVAL_MEMORY"
        with self.session_maker() as session:
            unique_data_sources = (
                session.query(self.db_model.data_source)
                .filter(*self.filters)
                .distinct()
                .all()
            )
...
class PostgresStorageConnector(SQLStorageConnector):
    ...
    def __init__(self, table_type: str, config: MemGPTConfig, user_id, agent_id=None):
        ...
        # create table
        self.db_model = get_db_model(
            config, self.table_name, table_type, user_id, agent_id
        )
        ...
        for c in self.db_model.__table__.columns:
            if c.name == "embedding":
                assert isinstance(
                    c.type, Vector
                ), f"Embedding column must be of type Vector, got {c.type}"
        ...
        Base.metadata.create_all(
            self.engine, tables=[self.db_model.__table__]
    ...
    def query(
        ...
        with self.session_maker() as session:
            results = session.scalars(
                ...
                .filter(*filters)
                .order_by(self.db_model.embedding.l2_distance(query_vec))
                .limit(top_k)
            ).all()
    ...
    def insert_many(
        self, records: List[RecordType], exists_ok=True, show_progress=False
    ):
            ...
            with self.engine.connect() as conn:
                ...
                if exists_ok:
                    upsert_stmt = stmt.on_conflict_do_update(
                        ...
                        },  # Replace with your primary key column
                    )
