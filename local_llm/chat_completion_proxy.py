...
from utils import count_tokens, get_tool_call_id, get_utc_time
...
def get_chat_completion(
    ...
    ), "Local LLM calls need the endpoint type (eg webui) to be explicitly set"
    ...
    try:
        if endpoint_type == "ollama":
            result, usage = get_ollama_completion(
                endpoint, auth_type, auth_key, model, prompt, context_window
            )
        else:
            raise LocalLLMError(
    ...
    try:
        ...
        print(
            json.dumps(chat_completion_result, indent=2, ensure_ascii=JSON_ENSURE_ASCII)
        )
    except Exception as e:
        raise LocalLLMError(
    ...
    if usage["completion_tokens"] is None:
        print(f"usage dict was missing completion_tokens, computing on-the-fly...")
...
def generate_grammar_and_documentation(
    ...
    for key, func in functions_python.items():
        grammar_function_models.append(
            create_dynamic_model_from_function(
                func, add_inner_thoughts=add_inner_thoughts_param_level
            )
        )
