...
def insert_heartbeat(message):
    ...
    if message_copy.get("function_call"):
        ...
        message_copy["function_call"]["arguments"] = json.dumps(
            params, ensure_ascii=JSON_ENSURE_ASCII
        )
        ...
        params = json.loads(params, strict=JSON_LOADS_STRICT)
        params["request_heartbeat"] = True
        message_copy["tools_calls"][0]["function"]["arguments"] = json.dumps(
            params, ensure_ascii=JSON_ENSURE_ASCII
        )
...
def heartbeat_correction(message_history, new_message):
    ...
    if message_history[-1]["role"] == "user":
        try:
            content = json.loads(
                message_history[-1]["content"], strict=JSON_LOADS_STRICT
            )
    ...
    if new_message["role"] == "assistant":
        if new_message.get("function_call") or new_message.get("tool_calls"):
            if new_message.get("function_call"):
                ...
                function_name = (
                    new_message.get("tool_calls")[0].get("function").get("name")
                )
...
def patch_function(message_history, new_message):
    corrected_output = heartbeat_correction(
        message_history=message_history, new_message=new_message
    )
